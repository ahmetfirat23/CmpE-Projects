...
public class AVL implements Tree{
    ...
    public AVL(String IP, FileWriter writer) {
    ...
    public void deleteNode(String IP) throws IOException {
        ...
        root = recDelete(root, IP, null, true);
    }

    Node recDelete(Node node, String IP, Node parent, boolean isLog) throws IOException{
        if (node==null){
            System.out.println("I was going to delete " + IP + " but i couldn't find anything after checking node " + parent.IP);
            return null;
        }
        else if(node.compareTo(IP)<0){
            node.right = recDelete(node.right, IP, node,isLog);
        }
        else if(node.compareTo(IP)>0){
            node.left = recDelete(node.left, IP, node, isLog);
        }
        else{ //Target found
            if(node.left==null && node.right==null){
                ...
                node.IP = min.IP;
                node.right = recDelete(node.right, min.IP, node, false);
            }
        }
        node.height = updatedHeight(node);
        node = rebalance(node);
    ...
    /**
     * Balance functionality will not be provided as instructions in the input file. Instead, your AVL tree needs to
     * rebalance itself as the new nodes are added and some nodes are deleted. Your AVL tree should never be
     * in a unbalanced situation. As you know, there are 2 types of rebalancing in AVL trees, single rotation and
     * double rotation. These are just reorganization of pointers in short. When the rotations happen, all nodes
     * involved in the rotation, 2 nodes in single rotation case and 3 nodes in double rotation case, will log the
     * rotation type as: Rebalancing <Rotation_type>
    ...
    private Node rebalance(Node node) throws IOException {
        ...
        // These rules can be derived by observing different trees
            ...
            Node child = leftRotate(node.left, node.left.right);
            ...
            Node child = rightRotate(node.right, node.right.left);
            node = leftRotate(node, child);
        }
        return node;
    }

    // Calculates balance factor for rotation
    // My convention is balance factor = height of left subtree - height of right subtree
    ...
    private int updatedHeight(Node node){
        if(node==null)
            return -1;
        else
    ...
    private Node leftRotate(Node parent, Node child){
        ...
        parent.height = updatedHeight(parent);
        child.height = updatedHeight(child);
