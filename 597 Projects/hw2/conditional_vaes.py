...
def train_conditional_vae(model, loader, num_epochs, learning_rate, beta=1.0, run_output_dir="."):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for x_batch, y_batch_idx in tqdm(loader, desc=f"Cond. Epoch {epoch}/{num_epochs}", leave=False):
            ...
            ep_recon += recon_l.item() * x_batch.size(0) # Optional tracking
            ep_kld += kld_l.item() * x_batch.size(0)   # Optional tracking
...
if __name__ == "__main__":
    ...
    conditional_loader = DataLoader(conditional_dataset, batch_size=32, shuffle=True, num_workers=2, pin_memory=torch.cuda.is_available())

    # --- HYPERPARAMETERS ---    
    # CONV VAE SETTINGS
    CHOSEN_CONDITIONAL_ARCH = "conv"
    best_latent_dim = 32  # Example, replace with actual from unconditional run
    conv_initial_size = 4 # Example
    conv_initial_channels = 64 # Example
    ...
    if CHOSEN_CONDITIONAL_ARCH == "conv":
        ...
        conditional_model = ConditionalRNNEncoderConvDecoderVAE(
            rnn_type=rnn_type, input_dim=28, hidden_dim_rnn=rnn_hidden_dim,
            num_layers_rnn=rnn_num_layers, latent_dim=best_latent_dim,
    ...
    # --- USER ACTION REQUIRED: Load your trained classifier from Assignment 1 ---
    # classifier_model = YourClassifierClass() # Instantiate your classifier
    # classifier_model.load_state_dict(torch.load("path/to/your/classifier_weights.pth"))
    # classifier_model.to(device).eval()
    classifier_model = None # Placeholder
    if classifier_model is None:
        tqdm.write("\nWARNING: Classifier model not loaded. Label predictions for generated samples will be skipped.")
    # --- END USER ACTION ---
    ...
    for new_label_idx in range(num_conditional_classes):
        ...
        if classifier_model:
            with torch.no_grad():
                # Ensure generated_samples are preprocessed if your classifier expects different input
                # (e.g., different normalization, different shape if classifier not for (B,1,28,28))
                ...
                for i in range(num_samples_per_category):
                    ...
                    # Map pred_idx (0,1,2) back to an original class name if your classifier outputs such indices
                    # This depends on how your classifier was trained and what its output represents
                    # For simplicity, just printing the index and confidence.
                    # If your classifier outputs scores for ALL original classes, you need a reverse map
                    # from its output index to the original class names.
                    # Here, we assume classifier predicts one of the 'num_conditional_classes'.
                    predicted_class_name = "Unknown"
                    if 0 <= pred_idx < len(target_class_names):
                         predicted_class_name = target_class_names[pred_idx]


                    tqdm.write(f"  Sample {i+1}: Predicted as '{predicted_class_name}' (index {pred_idx}), Confidence: {probabilities[i, pred_idx].item():.3f}")
