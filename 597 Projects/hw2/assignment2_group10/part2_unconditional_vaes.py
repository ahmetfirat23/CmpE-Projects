...
def save_image_grid(images_tensor, filepath, n_rows = 4, normalize = True, scale_each = True):
    ...
    actual_n_rows = min(n_rows, images_tensor.size(0))
...
class RNNEncoderConvDecoderVAE(nn.Module): 
    def __init__(self,  
                 ...
                 bidirectional_encoder=False, conv_decoder_initial_channels=64,  
        ...
        self.decoder_fc_in_features = conv_decoder_initial_channels*conv_decoder_initial_size*conv_decoder_initial_size 

        self.encoder_rnn = rnn_module(self.input_dim_rnn, 
                                      hidden_dim_rnn,
                                      num_layers_rnn,
                                      batch_first=True,
...
if __name__ == "__main__":
    ...
    for grid_family_name, param_grid in all_param_grids_with_names.items():
        ...
        tqdm.write(f"Best config for {grid_family_name}: {best_model_final_config_for_family}") 
        tqdm.write(f"Loading weights: {best_model_weights_path_for_family}")
        best_model_arch = best_model_final_config_for_family['model_arch']
        best_latent_dim = best_model_final_config_for_family['latent_dim']
        if best_model_arch=='rnn_conv':
            final_best_model_for_family = RNNEncoderConvDecoderVAE(
                rnn_type=best_model_final_config_for_family['rnn_type'], 
                hidden_dim_rnn=best_model_final_config_for_family['hidden_dim'], 
                num_layers_rnn=best_model_final_config_for_family['num_layers'], 
                latent_dim=best_latent_dim, 
                encoder_variant=best_model_final_config_for_family['encoder_variant'], 
                bidirectional_encoder=best_model_final_config_for_family.get('bidirectional', False)
                ).to(device)
        ...
        generate_and_save_samples(final_best_model_for_family, N_FID_REAL_SAMPLES, samples_filepath)
        is_mean, is_std, fid_score = -1.0, -1.0, -1.0 

        final_best_model_for_family.eval()
        with torch.no_grad():
            z_metric = torch.randn(N_FID_REAL_SAMPLES, final_best_model_for_family.latent_dim).to(device)
        ...
        is_metric_calc.update(gen_imgs_for_metric.to(device))
        is_mean_t, is_std_t = is_metric_calc.compute()
        is_mean, is_std = is_mean_t.item(), is_std_t.item()
        fid_metric_calc.update(fid_real_images_for_metric_global.to(device), real=True)
        fid_metric_calc.update(gen_imgs_for_metric.to(device), real=False)
        ...
        summary_for_reporting = { 
            'config':best_model_final_config_for_family,
            'best_val_loss':best_val_loss_for_family,
            'weights_path':best_model_weights_path_for_family,
            'output_dir':best_model_output_dir_for_family,
            'is_mean':is_mean,
            'is_std':is_std,
            'fid_score':fid_score}
        overall_best_models_summary[grid_family_name] = summary_for_reporting
        summary_file_path = os.path.join(best_model_output_dir_for_family,"BEST_MODEL_summary.json")
        with open(summary_file_path, 'w') as f:
            json.dump(summary_for_reporting, f, indent=4)
        tqdm.write(f"Comprehensive summary for best {grid_family_name} model saved to: {summary_file_path}")

    overall_grid_pbar.close()

    tqdm.write("\n\n--- Grid Search Summary (Best from each family) ---")
    for family_name, summary in overall_best_models_summary.items():
        tqdm.write(f"\nBest for {family_name}:")
        ...
        tqdm.write(f"  Full details in: {os.path.join(summary['output_dir'],  'BEST_MODEL_summary.json')}")
