...
def normalize_np(x, dim=-1, eps=1e-12):
...
class CosineSimilarityLoss():
    def __call__(self, a, b):
        ...
        cos_sim = np.sum(a_hat * b_hat, axis=1, keepdims=True)  # (N, 1)
    ...
    def backward(self, a, b):
        ...
        # Gradients
        grad_a = (b_hat - cos_sim * a_hat) / a_norm
        grad_b = (a_hat - cos_sim * b_hat) / b_norm
...
class SGD():
    ...
    def step_w_grad(self, grad): # for contrastive loss.
        prop_grad = grad
        for idx, l in enumerate(self.model.cache[::-1]):
            prop_grad = l.backprop(prop_grad)
            if type(l) == Linear:
