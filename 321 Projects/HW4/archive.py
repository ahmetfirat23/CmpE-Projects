...
def create_type(catalogue, type_name, args):
    ...
    try:
        ...
        print(f"Error: {type_name} is not a valid type")
        raise Exception
    ...
    for pair in pairs:
        ...
        if pair[1] not in ['int', 'str']:
            print(f"Error: {pair[1]} is not a valid type")
            raise Exception
...
def create_record(catalogue, type_name, args):
    ...
    if type_name not in catalogue:
        print(f"Error: Type {type_name} does not exist")
        raise Exception
    # If there are not enough fields
    if len(args) != len(catalogue[type_name]['fields']):
        print(f"Error: {type_name} is not a valid record")
        raise Exception
    # Create the record
    record = {}
    ...
    if primary_key in lookup:
        print(f"Error: {primary_key} already exists")
        raise Exception
...
def delete_record(catalogue, type_name, args):
    ...
    if primary_key not in lookup:
        print(f"Error: {primary_key} does not exist")
        raise Exception
...
def process_command(catalogue, command):
    ...
    try:
        ...
        if len(args) < 4:
            print(f"Error: {command.strip()} is not a valid command")
            raise Exception
        ...
        with open("log.csv", 'a') as f:
            writer = csv.writer(f)
            writer.writerow([command_time, command.strip(), "success"])
    except:
        # Log the command and failure
        with open("log.csv", 'a') as f:
            writer = csv.writer(f)
            writer.writerow([command_time, command.strip(), "failure"])
