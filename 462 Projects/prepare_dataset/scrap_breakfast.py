...
def extract_data(url):
    ...
    for day in tree.xpath('//td[@class="single-day past"]'):
        ...
        meals = []
        for meal in day.xpath('.//div[@class="view-item view-item-kahvalt_men_s_"]'):
            meal_info = {}
            
            # Extract meal time (e.g., breakfast)
...
if __name__ == '__main__':
    menu_data = []
    # for i in range(4, 10):  
    #     raw_data = extract_data(f"https://yemekhane.bogazici.edu.tr/kahvalti-menu/2024-0{i}")
    #     processed_data = process_data(raw_data)
    #     menu_data += processed_data
    raw_data = extract_data(f"https://yemekhane.bogazici.edu.tr/kahvalti-menu/2024-09")
    processed_data = process_data(raw_data)
    menu_data += processed_data
    raw_data = extract_data(f"https://yemekhane.bogazici.edu.tr/kahvalti-menu/2024-10")
    processed_data = process_data(raw_data)
    menu_data += processed_data
    with open('menu_data_breakfast_910.json', 'w', encoding='utf-8') as json_file:
        json.dump(menu_data, json_file, indent=4)

    # Optional: If you need to save the original data with Turkish characters
